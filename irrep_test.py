...
def dummy_data_for_multiple_irreps(l: list[int]) -> torch.Tensor:
    # creates dummy data for a specific l value
    arr = torch.zeros(2*l+1)
    arr[0] = 1 # so the array is normalized to length 1
    return arr
...
def test_irrep_id():
    assert Irrep(l=1, parity=1, data=dummy_data(1)).id() == "1e"
    assert Irrep(l=1, parity=-1, data=dummy_data(1)).id() == "1o"
    assert Irrep(l=2, parity=1, data=dummy_data(2)).id() == "2e"
    assert Irrep(l=2, parity=-1, data=dummy_data(2)).id() == "2o"
...
def test_irreps_id():
    ...
    assert Irreps.from_id("1x1e + 1x1e").id() == "2x1e" # test consolidating irreps
...
def test_irrep_tensor_product():
    ...
    assert Irreps.from_id("1x0e+1x1o").tensor_product(Irreps("1x0e+1x1o")).id() == "2x0e+2x1o+1x1e+1x2e"

    # this next tensor product test is here just to ensure my code works
    assert Irreps.from_id("1x0e+1x1o+1x2e").tensor_product(Irreps("1x0e+1x1o+1x2e")).id() == "3x0e + 4x1o + 2x1e + 2x2o + 4x2e + 2x3o + 1x3e + 1x4e"